defmodule Jido.Integration.V2.Auth.Persistence.Resolution do
  @moduledoc "Resolved auth persistence profile and store modules."
...
defmodule Jido.Integration.V2.Auth.Persistence.Owner do
  @moduledoc false
...
defmodule Jido.Integration.V2.Auth.Persistence do
  @moduledoc """
  Persistence policy resolver for auth-owned stores.
  """
  ...
  def configure!(attrs \\ []) do
    ...
    case __MODULE__.Owner.put(resolution) do
      ...
      {:error, :not_started} ->
        raise ArgumentError,
              "auth persistence owner is not started; start Jido.Integration.V2.Auth.Application before configuring persistence"
    end
  end
